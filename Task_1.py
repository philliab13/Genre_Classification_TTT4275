...
feat_dict = get_features_dict(data, ["Track ID", "spectral_rolloff_mean",
                              "mfcc_1_mean", "spectral_centroid_mean", "tempo", "GenreID"])
...
plot_cm(cm, genre_names, "cm_no_normalizing")

# Performance with normalizing
prediction_testing_points_norm = knn_predict(
    norm_training_data, training_labels, norm_testing_data, 5)

# You get recall, precision, f1-score from this, this could also be done by looking at the confusion matrix and using the formulas from lecture 18
report_norm = manual_classification_report(
    testing_labels, prediction_testing_points_norm, [str(cls) for cls in classes])
print(report_norm)


cm_norm = metrics.confusion_matrix(
    testing_labels, prediction_testing_points_norm)

plot_cm(cm_norm, genre_names, "cm_minmax_normalizing")
...
plot_cm(cm_norm_z, genre_names, "cm_z_normalizing")
